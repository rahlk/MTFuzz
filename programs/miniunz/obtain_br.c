...
int exec_tmout = 140;                  /* Exec timeout (ms)                 */
...
static u8 run_target_br(int timeout) {
...
int compareName(const void *const A, const void *const B){
    int diff = atoi((*(struct dirent **) A)->d_name) - atoi((*(struct dirent **) B)->d_name);
    if(diff == 0)
        return 0;
    if(diff > 0)
        return 1;
...
void obtain_br(char* dir, int stage){
    DIR *dp;
    struct dirent *entry;
    struct stat statbuf;
    ...
    while((entry = readdir(dp)) != NULL) {
        ...
        if(S_ISREG(statbuf.st_mode)) {
            ...
            if(tmp != entry->d_name){
                ...
                int fault = run_target_br(exec_tmout); 
                if(fault == FAULT_CRASH){
                    time_t now;
                    struct tm *tm;
                    ...
                    if((tm = localtime (&now)) == NULL) {
                        ...
                        char* mut_fn = alloc_printf("%s/crash_%d_%02d_%02d:%02d:%02d\n", "./crashes", cnt, tm->tm_mday,
                ...
                if(((int *)trace_bits)[3] == 12)
                    printf("%s:%d %d\n",entry->d_name, ((int *)trace_bits)[1],((int *)trace_bits)[2]);
                else{
                    remove(entry->d_name);
                }          
            }
...
void main(int argc, char*argv[]){
    int opt;
    while ((opt = getopt(argc, argv, "+i:o:l:t:")) > 0)
    ...
    switch (opt) {
      ...
      case 't': /* target br_id */
         sscanf (optarg,"%d",&target_br_id);
         printf("target br_id: %d\n", target_br_id);
         break;
