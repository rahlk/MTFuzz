...
def process_data():
    ...
    for f in vari_seeds:
        if './seeds/'+f.split('/')[-1] in seed_list:
            continue
        try:
            subprocess.call(['./afl-tmin','-i',f,'-o', './seeds/'+f.split('/')[-1], '-k', str(MAX_FILE_SIZE)]+argvv,stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=30)
