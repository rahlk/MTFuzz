      ...
      for (auto &I :BB){
        if(CmpInst* cmp_inst = dyn_cast<CmpInst>(&I)){
            ...
            if(auto* type = dyn_cast<IntegerType>(op1->getType())){
              ...
              switch(type->getBitWidth()){
                  case 8:
                      ...
                      errs()<<"$$$### br_id "<< cnt << " br_type " << cmp_opcode << " constant_loc " << constantLoc << " constant_val " << constantVal << " len 1\n";  
                      }
                      break;
                  case 16:
                      {
                      ...
                      errs()<<"$$$### br_id "<< cnt << " br_type " << cmp_opcode << " constant_loc " << constantLoc << " constant_val " << constantVal << " len 2\n";  
                      }
                      break;
                  case 32:
                      {
                      ...
                      errs()<<"$$$### br_id "<< cnt << " br_type " << cmp_opcode << " constant_loc " << constantLoc << " constant_val " << constantVal << " len 4\n";  
                      }
                      break;
                  case 64:
                      {
                      ...
                      errs()<<"$$$### br_id "<< cnt << " br_type " << cmp_opcode << " constant_loc " << constantLoc << " constant_val " << constantVal << " len 8\n";  
                      }
                      break;
                  default:
                      break;
            ...
            switch(type->getBitWidth()){
              case 8:
                  ...
                  for (int i = 0; i< num_cases;i++){
                    ...
                    Value* args[] = {br_id, type, op1, op2, constant_loc};  
                    IRB.CreateCall(log_br8,args); 
                    errs()<<"$$$### br_id "<< cnt << " br_type 11 constant_loc 2 constant_val " << cast<ConstantInt>(op2)->getZExtValue() << " len 1\n";  
                  }
                  }
                  break;
              case 16:
                  ...
                  for (int i = 0; i< num_cases;i++){
                    ...
                    Value* args[] = {br_id, type, op1, op2, constant_loc};  
                    IRB.CreateCall(log_br16,args); 
                    errs()<<"$$$### br_id "<< cnt << " br_type 11 constant_loc 2 constant_val " << cast<ConstantInt>(op2)->getZExtValue() << " len 2\n";  
                  }
                  }
                  break;
              case 32:
                  ...
                  for (int i = 0; i< num_cases;i++){
                    ...
                    Value* args[] = {br_id, type, op1, op2, constant_loc};  
                    IRB.CreateCall(log_br32,args); 
                    errs()<<"$$$### br_id "<< cnt << " br_type 11 constant_loc 2 constant_val " << cast<ConstantInt>(op2)->getZExtValue() << " len 4\n";  
                  }
                  }
                  break;
              case 64:
                  ...
                  for (int i = 0; i< num_cases;i++){
                    ...
                    Value* args[] = {br_id, type, op1, op2, constant_loc};  
                    IRB.CreateCall(log_br64,args); 
                    errs()<<"$$$### br_id "<< cnt << " br_type 11 constant_loc 2 constant_val " << cast<ConstantInt>(op2)->getZExtValue() << " len 8\n";  
                  }
                  }
                  break;
              default:
		  ...
		  tmp = string_to_hex(hope1->getRawDataValues());
                  errs()<<"$$$### br_id "<< cnt << " br_type 10 constant_loc 1 constant_val " << tmp << " len " << tmp.length()/2 << "\n";  
                }
                else if( auto hope1 = dyn_cast<ConstantAggregateZero>((cast<GlobalVariable>(hope->getOperand(0)))->getInitializer())){ 
                  errs()<<"$$$### br_id "<< cnt << " br_type 10 constant_loc 1 constant_val 00 len 0\n"; 
                }
              }
                ...
                if ( auto hope1 = dyn_cast<ConstantDataArray>((cast<GlobalVariable>(hope->getOperand(0)))->getInitializer())){
		  tmp = string_to_hex(hope1->getRawDataValues());
                  errs()<<"$$$### br_id "<< cnt << " br_type 10 constant_loc 2 constant_val " << tmp << " len " << tmp.length()/2 << "\n";  
                }
                // len 0 means zero initizlization.
                else if( auto hope1 = dyn_cast<ConstantAggregateZero>((cast<GlobalVariable>(hope->getOperand(0)))->getInitializer())){  
                  errs()<<"$$$### br_id "<< cnt << " br_type 10 constant_loc 2 constant_val 00 len 0\n"; 
                }
              }
              // constant_loc 0 means no magic constant
              else
                  errs()<<"$$$### br_id "<< cnt << " br_type 10 constant_loc 0 constant_val 00 len 0 \n";  
            ...
            if(fun->getName().equals("strncmp")){
              ...
              if(auto hope = dyn_cast<ConstantExpr>(op1)){
                ...
                if ( auto hope1 = dyn_cast<ConstantDataArray>((cast<GlobalVariable>(hope->getOperand(0)))->getInitializer())){
		  tmp = string_to_hex(hope1->getRawDataValues());
                  errs()<<"$$$### br_id "<< cnt << " br_type 12 constant_loc 1 constant_val " << tmp << " len " << tmp.length()/2 << "\n";  
                }
                else if( auto hope1 = dyn_cast<ConstantAggregateZero>((cast<GlobalVariable>(hope->getOperand(0)))->getInitializer()))
                  errs()<<"$$$### br_id "<< cnt << " br_type 12 constant_loc 1 constant_val 00 len 0\n"; 
              }
              else if(auto hope = dyn_cast<ConstantExpr>(op2)){
                constant_loc =  ConstantInt::get(Int32Ty, 2);
                std::string tmp;
                if ( auto hope1 = dyn_cast<ConstantDataArray>((cast<GlobalVariable>(hope->getOperand(0)))->getInitializer())){
		  tmp = string_to_hex(hope1->getRawDataValues());
                  errs()<<"$$$### br_id "<< cnt << " br_type 12 constant_loc 2 constant_val " << tmp << " len " << tmp.length()/2 << "\n";  
                }
                else if( auto hope1 = dyn_cast<ConstantAggregateZero>((cast<GlobalVariable>(hope->getOperand(0)))->getInitializer()))
                  errs()<<"$$$### br_id "<< cnt << " br_type 12 constant_loc 2 constant_val 00 len 0\n"; 
              }
              // constant_loc 0 means no magic constant
              else
                  errs()<<"$$$### br_id "<< cnt << " br_type 10 constant_loc 0 constant_val 00 len 0 \n";  
